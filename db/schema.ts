...
export const reservations = sqliteTable(
  ...
  (table) => [uniqueIndex("idx_reservations_code").on(table.code)],
);
...
export const transactions = sqliteTable(
  "transactions",
  {
    ...
    location: text("location").notNull(),
    status: text("status").notNull().default("Plaćeno"),
    paymentMethod: text("payment_method").notNull().default("Kartica"),
    notes: text("notes").notNull().default(""),
    createdAt: text("created_at")
      .notNull()
      .default(sql`CURRENT_TIMESTAMP`),
...
export const documents = sqliteTable(
  "documents",
  {
    ...
    uploadedBy: text("uploaded_by").notNull().default("Marko Nikolić"),
...
export const teamUsers = sqliteTable(
  "team_users",
  {
    ...
    status: text("status").notNull().default("Pozvan"),
    permissions: text("permissions").notNull().default("[]"),
    lastActive: text("last_active").notNull().default("Još nije aktivan"),
    createdAt: text("created_at")
      .notNull()
      .default(sql`CURRENT_TIMESTAMP`),
  },
...
export const deletedRecords = sqliteTable(
  "deleted_records",
  {
    ...
    deletedAt: text("deleted_at")
      .notNull()
      .default(sql`CURRENT_TIMESTAMP`),
    deletedBy: text("deleted_by").notNull().default("Marko Nikolić"),
  },
  (table) => [
    uniqueIndex("idx_deleted_records_entity_key").on(
      table.entity,
      table.recordKey,
    ),
  ],
...
export const vehicleDamages = sqliteTable(
  "vehicle_damages",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    code: text("code").notNull(),
    ...
    date: text("date").notNull(),
    cost: integer("cost").notNull().default(0),
    status: text("status").notNull().default("Prijavljena"),
    notes: text("notes").notNull().default(""),
    createdAt: text("created_at")
      .notNull()
      .default(sql`CURRENT_TIMESTAMP`),
